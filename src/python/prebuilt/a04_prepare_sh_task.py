    ...
    for root, dirs, files in os.walk(directory_npp):
        ...
        for filename in files:
            if filename.endswith("P.pkl"):
                ...
                if n_problems>0:
                    time_limit_sh += n_problems*time_limit + 610 # time limit for the *.sh script (config for server)
                    ...
                    if not grouped:
                        ...
                        with open(file_sh, 'w') as f:
                            ...
                            f.write('\n'.join(preamble_sh(cpu, ram, h, m, s, *args) + [command,] + ['sleep 600', ] ))
        ...
        with open(file_sh, 'w') as f:
            f.write('\n'.join(preamble_sh(cpu, ram, h, m, s, *args) + command_list_sh+['sleep 600', ] ))
            logger.info(f'File created : {file_sh}')
